...
Coroutine 은 스레드 내에서 실행되며 특정 Coroutine 이 실행중에 IO 요청을 수행하고 응답을 대기 중 일때 중단됩니다. 스레드가 중단되었을 때 코틀린은 뒷단에서 다른 코루틴 또는 스레드 작업을 수행합니다. 그리고 IO 요청에 대한 응답이 와서 처리를 해야 할 때 코루틴은 다시 재개되는데, 이때 원래 IO요청을 수행했던 스레드가 아닌 다른 스레드에서 실행될 수도 있습니다.<br/>

이렇게 코루틴이 작업을 중단하고 다시 재개할 때 어떤 스레드에서 실행될지를 결정하는 역할을 하는 것은 코루틴 디스패처(CoroutineDispatcher)입니다.<br/>

즉, 코틀린은 스레드 하나에서만 작업이 쭉 이뤄지지는 않습니다. 그리고 코루틴이 실행될 스레드를 결정하는 것은 코루틴 디스패처(CoroutineDispatcher) 입니다.<br/>

 코틀린은 뒷단에서 다른 스레드를 수행합니다. IO 요청에 대한 응답이 왔을 때 중단 되었던 Coroutine 은 원래 수행됐던 스레드가 아닌 다른 스레드에서 수행될 수 있습니다.<br/>
